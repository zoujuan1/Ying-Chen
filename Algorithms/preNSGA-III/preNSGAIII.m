...
%--------------------------------------------------------------------------
    ...
    %% Parameter setting
    [point,r] = Global.ParameterSet(zeros(1,Global.M)+0.5,0.2);
    ...
    Zmin         = min(FixObj(all(Population.cons<=0,2),1:end),[],1);
    ...
    while Global.NotTermination(Population)
     ...
     %% 距离惩罚
        ...
        Zmin       = min([Zmin;FixObj_offspring(all(Offspring.cons<=0,2),1:end)],[],1);
        Population = preEnvironmentalSelection([Population,Offspring],Global.N,Z,Zmin,[FixObj;FixObj_offspring]);
