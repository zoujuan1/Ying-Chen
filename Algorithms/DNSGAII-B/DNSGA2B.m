...
% %         Draw(Pt1.objs,'k+','Markeredgecolor',[.6 .0 .0],'Markerfacecolor',[.0 .9 .0],'MarkerSize',3);
        ...
        if Change(Global)   % 检测环境变化
            t=t+1   
            num=floor(Global.N*0.2);  %随机选0.2的个体去变异
            Pt2=Population;
            

            
            rand=randperm(Global.N);
            index=rand(1:num);
            Parent=Pt2(index);       %变异父本
            
            offpring=EApermutation(Global,Parent);  %变异后的个体
            for i =1:num          %替换
                Pt2(index(i))=offpring(i);
            end
            Decs=Pt2.decs;
