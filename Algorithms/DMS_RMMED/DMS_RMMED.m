...
%         Draw(Pt1.objs,'k+','Markeredgecolor',[.6 .0 .0],'Markerfacecolor',[.0 .9 .0],'MarkerSize',3);
        ...
        if Change(Global)  % 检测环境变化
            ...
            [Population,FrontNo,CrowdDis] = EnvironmentalSelection([Population,Pachive],Global.N);
            PF=Global.PF;
            if t>=2      %除去第一次环境变化
                 ...
                 score2=GD(Population.objs,Global.PF);      %计算GD
                 metric2=[metric2,score2];
                 mgd=sum(metric2)/length(metric2); %计算MGD
                 gd=mgd;                         %将MGD通过全局变量gd传给指标函数MGD() 
            end
        else
