...
%--------------------------------------------------------------------------
    properties
        ...
        tao_n   = 30;                   %环境变化的频率，tao_t越大越接近静态问题
        NT=10;                           %环境变化剧烈程度,nt越小变化越大
    end
    ...
    properties(SetAccess = private)
        ...
        problem    = @DTLZ1;            % Problem function
        ...
        mode       = 3;                 % Run mode (1.show result 2.save result 3.run outputFcn)
    ...
    methods
        ...
        %       obj.NotTermination(Population)
            ...
            drawnow();
            obj.PF = obj.problem('PF',obj,10000);
%             DF(func2str(obj.problem),obj.NT); 
            obj.outputFcn(obj);
        ...
        %% changed of reference point
        function change = Change(obj)
        change = rem(obj.gen, 30) == 0;
        end
...
%                 uimenu(top,'Label','Result (DF)',     'CallBack',{@(hObject,~,obj,P)eval('cla;Draw(P.decs);GLOBAL.cb_menu(hObject);'),obj,Population});
